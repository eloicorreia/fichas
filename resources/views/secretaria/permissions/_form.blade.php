...
<div class="grid gap-6 lg:grid-cols-2">
    ...
    <div>
        ...
        <input id="label" name="label" type="text" value="{{ old('label', $permission->label) }}"
            class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm text-slate-800 outline-none transition focus:border-sky-400 focus:ring-2 focus:ring-sky-100">
        @error('label') <p class="mt-1 text-sm text-rose-600">{{ $message }}</p> @enderror
    </div>

    <div>
        <label class="mb-2 block text-sm font-semibold text-slate-700" for="module">Módulo</label>
        <input id="module" name="module" type="text" value="{{ old('module', $permission->module) }}"
            class="w-full rounded-2xl border border-slate-300 px-4 py-3 text-sm text-slate-800 outline-none transition focus:border-sky-400 focus:ring-2 focus:ring-sky-100">
        @error('module') <p class="mt-1 text-sm text-rose-600">{{ $message }}</p> @enderror
    </div>

    <div class="flex items-center gap-3 pt-8">
        <input name="active" type="hidden" value="0">
        <input id="active" name="active" type="checkbox" value="1" @checked((bool) old('active', $permission->active ?? true))
            class="h-4 w-4 rounded border-slate-300 text-sky-700 focus:ring-sky-500">
        <label for="active" class="text-sm font-medium text-slate-700">Permissão ativa</label>
    </div>
</div>

<div class="mt-8 flex items-center justify-end gap-3">
    <a href="{{ route('secretaria.permissions.index') }}"
